...
function RegistrationPage() {
  ...
  const getFlow = useCallback((flowId: string) => {
    console.log('ini di RegistrationPage getFlow dengan flowid:', flowId)
    return (
      sdk
  ...
  const createFlow = () => {
    sdk
      ...
      .then(({ data: flow }) => {
        console.log('RegistrationPage createFlow flow:', flow)
        ...
        if ('continue_with' in data) {
          ...
          for (const cw of data.continue_with ?? []) {
            if (cw.action === 'show_verification_ui') {
              console.log(
                'ini di submit flow register page Redirecting to verification page with cw flow id',
                cw.flow.id,
              )
              const search = new URLSearchParams()
              search.set('flow', cw.flow.id)
              search.set('type', 'verification')
              navigate(
                {
